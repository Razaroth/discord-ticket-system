...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        // ── Button: Continue to Pre-Test (opens dropdown checklist) ────────
        if (interaction.isButton() && interaction.customId === 'continue_pretest') {
            ...
            // Initialize pretest draft with empty selections
            const pretestDraftKey = `${interaction.guildId}:${interaction.user.id}:pretest`;
            pendingTicketDrafts.set(pretestDraftKey, {
                displayTouch: null,
                buttonsPorts: null,
                cameraAudio: null,
                powerBattery: null,
            ...
            const row = new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId('pretest_display_touch_select')
                    .setPlaceholder('Select display & touch condition...')
                    .addOptions([
                        { label: '✅ Good / No issues', value: 'Good', description: 'Screen and touch working normally' },
                        { label: '⚠️ Minor cracks', value: 'Minor cracks', description: 'Small cracks visible but functioning' },
                        { label: '⚠️ Major cracks', value: 'Major cracks', description: 'Significant cracks affecting display' },
                        { label: '❌ Shattered', value: 'Shattered', description: 'Screen heavily damaged or shattered' },
                        { label: '❌ No response', value: 'No response', description: 'Touch or display not responding' },
                    ]),
            );

            await interaction.reply({
                content: '**Step 4 of 4** — First-Look Pre-Test Checklist\n\n**Display & Touch State** — What is the condition of the display?',
                components: [row],
                ephemeral: true,
            });
            return;
        ...
        // ── Select: Pre-Test Display & Touch ──────────────────────────────
        if (interaction.isStringSelectMenu() && interaction.customId === 'pretest_display_touch_select') {
            const pretestDraftKey = `${interaction.guildId}:${interaction.user.id}:pretest`;
            const pretestDraft = pendingTicketDrafts.get(pretestDraftKey);
            if (!pretestDraft) {
                await interaction.reply({
                    content: '❌ Pre-test expired. Please start again.',
                    ephemeral: true,
                });
                return;
            }

            pretestDraft.displayTouch = interaction.values[0];

            const row = new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId('pretest_buttons_ports_select')
                    .setPlaceholder('Select buttons & ports condition...')
                    .addOptions([
                        { label: '✅ All working', value: 'All working', description: 'All buttons and ports function properly' },
                        { label: '⚠️ Buttons sticky', value: 'Buttons sticky', description: 'Some buttons are sticky or slow' },
                        { label: '⚠️ Port loose', value: 'Port loose', description: 'Charging or audio port is loose' },
                        { label: '❌ Port damaged', value: 'Port damaged', description: 'Port is visibly damaged' },
                        { label: '❌ Multiple issues', value: 'Multiple issues', description: 'Multiple button/port issues present' },
                    ]),
            );

            await interaction.update({
                content: '**Step 4 of 4** — First-Look Pre-Test Checklist\n\n**Buttons & Ports State** — What is the condition of buttons and ports?',
                components: [row],
            });
            return;
        }

        // ── Select: Pre-Test Buttons & Ports ──────────────────────────────
        if (interaction.isStringSelectMenu() && interaction.customId === 'pretest_buttons_ports_select') {
            const pretestDraftKey = `${interaction.guildId}:${interaction.user.id}:pretest`;
            const pretestDraft = pendingTicketDrafts.get(pretestDraftKey);
            if (!pretestDraft) {
                await interaction.reply({
            ...
            pretestDraft.buttonsPorts = interaction.values[0];

            const row = new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId('pretest_camera_audio_select')
                    .setPlaceholder('Select camera & audio condition...')
                    .addOptions([
                        { label: '✅ All working', value: 'All working', description: 'Cameras and audio functioning properly' },
                        { label: '⚠️ Rear camera damaged', value: 'Rear camera damaged', description: 'Back camera has issues' },
                        { label: '⚠️ Front camera damaged', value: 'Front camera damaged', description: 'Front camera has issues' },
                        { label: '⚠️ Speaker muffled', value: 'Speaker muffled', description: 'Speaker audio is muffled' },
                        { label: '❌ No audio', value: 'No audio', description: 'Speaker or mic not working' },
                    ]),
            );

            await interaction.update({
                content: '**Step 4 of 4** — First-Look Pre-Test Checklist\n\n**Camera & Audio State** — What is the condition of cameras and audio?',
                components: [row],
            });
            return;
        }

        // ── Select: Pre-Test Camera & Audio ───────────────────────────────
        if (interaction.isStringSelectMenu() && interaction.customId === 'pretest_camera_audio_select') {
            const pretestDraftKey = `${interaction.guildId}:${interaction.user.id}:pretest`;
            const pretestDraft = pendingTicketDrafts.get(pretestDraftKey);
            if (!pretestDraft) {
                await interaction.reply({
            ...
            pretestDraft.cameraAudio = interaction.values[0];

            const row = new ActionRowBuilder().addComponents(
                new StringSelectMenuBuilder()
                    .setCustomId('pretest_power_battery_select')
                    .setPlaceholder('Select power & battery condition...')
                    .addOptions([
                        { label: '✅ Normal', value: 'Normal', description: 'Powers on and charges normally' },
                        { label: '⚠️ Slow boot', value: 'Slow boot', description: 'Takes a long time to power on' },
                        { label: '⚠️ Battery drains quickly', value: 'Battery drains quickly', description: 'Battery depletes rapidly' },
                        { label: '⚠️ Charging issues', value: 'Charging issues', description: 'Charges slowly or inconsistently' },
                        { label: '❌ Won\'t power on', value: 'Wont power on', description: 'Device does not turn on' },
                    ]),
            );

            await interaction.update({
                content: '**Step 4 of 4** — First-Look Pre-Test Checklist\n\n**Power & Battery State** — What is the condition of power and battery?',
                components: [row],
            });
            return;
        }

        // ── Select: Pre-Test Power & Battery ──────────────────────────────
        if (interaction.isStringSelectMenu() && interaction.customId === 'pretest_power_battery_select') {
            const pretestDraftKey = `${interaction.guildId}:${interaction.user.id}:pretest`;
            const pretestDraft = pendingTicketDrafts.get(pretestDraftKey);
            if (!pretestDraft) {
                await interaction.reply({
            ...
            pretestDraft.powerBattery = interaction.values[0];

            // All selections complete - proceed to ticket creation
            const draftKey = `${interaction.guildId}:${interaction.user.id}`;
            const draft = pendingTicketDrafts.get(draftKey);
            if (!draft) {
                await interaction.reply({
            ...
            const preTestChecklist = {
                displayTouch: pretestDraft.displayTouch,
                buttonsPorts: pretestDraft.buttonsPorts,
                cameraAudio: pretestDraft.cameraAudio,
                powerBattery: pretestDraft.powerBattery,
                additionalNotes: '',
            };
            ...
            try {
                ...
                pendingTicketDrafts.delete(pretestDraftKey);
            } catch (err) {
